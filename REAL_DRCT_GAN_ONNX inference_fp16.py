...
def main():
    ...
    parser.add_argument(
        "--model_path", type=str, default="model.onnx", help="Path to the ONNX model"
    )
    parser.add_argument(
        "--input", type=str, default="input", help="Input folder with images"
    )
    parser.add_argument("--output", type=str, default="output", help="Output folder")
    parser.add_argument("--scale", type=int, default=4, help="Upscaling factor")
    parser.add_argument(
        "--tile_size", type=int, default=512, help="Tile size for processing"
    )
    ...
    for image_path in tqdm(
        glob.glob(os.path.join(args.input, "*")), desc="Processing images", unit="image"
    ):
        ...
        output_img = tile_process(
            img, ort_session, input_name, args.scale, args.tile_size, args.tile_pad
        )
