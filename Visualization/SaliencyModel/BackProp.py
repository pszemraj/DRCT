...
from SaliencyModel.utils import IG_baseline, interpolation, isotropic_gaussian_kernel
...
def I_gradient(
    numpy_image, baseline_image, model, attr_objective, fold, interp="linear"
):
    interpolated = interpolation(numpy_image, baseline_image, fold, mode=interp).astype(
        np.float32
    )
...
def GaussianBlurPath(sigma, fold, l=5):
    def path_interpolation_func(cv_numpy_image):
        ...
        return np.moveaxis(image_interpolation, 3, 1).astype(np.float32), np.moveaxis(
            lambda_derivative_interpolation, 3, 1
        ).astype(np.float32)
...
def GaussianLinearPath(sigma, fold, l=5):
    def path_interpolation_func(cv_numpy_image):
        ...
        image_interpolation = interpolation(
            cv_numpy_image, baseline_image, fold, mode="linear"
        ).astype(np.float32)
        lambda_derivative_interpolation = np.repeat(
            np.expand_dims(cv_numpy_image - baseline_image, axis=0), fold, axis=0
        )
        return np.moveaxis(image_interpolation, 3, 1).astype(np.float32), np.moveaxis(
            lambda_derivative_interpolation, 3, 1
        ).astype(np.float32)
...
def Path_gradient(
    numpy_image, model, attr_objective, path_interpolation_func, cuda=False
):
    """
...
def saliency_map_I_gradient(
    ...
    :return:
    """
    numpy_baseline = np.moveaxis(
        IG_baseline(np.moveaxis(numpy_image, 0, 2) * 255.0, mode=baseline) / 255.0, 2, 0
    )
