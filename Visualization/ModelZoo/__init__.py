...
NN_LIST = ["RCAN", "CARN", "RRDBNet", "RNAN", "SAN", "DRCT", "SwinIR", "HAT"]
...
def print_network(model, model_name):
    ...
    print(
        "Network [%s] was created. Total number of parameters: %.1f kelo. "
        "To see the architecture, do print(network)." % (model_name, num_params / 1000)
    )
...
def load_model(model_loading_name):
    ...
    if len(splitting) == 1:
        ...
        raise NotImplementedError()
    assert (
        model_name in NN_LIST or model_name in MODEL_LIST.keys()
    ), "check your model name before @"
    ...
    if model_name == "DRCT":
        ...
        param_key_g = "params"
        net.load_state_dict(
            state_dict[param_key_g] if param_key_g in state_dict.keys() else state_dict,
            strict=True,
        )
    else:
